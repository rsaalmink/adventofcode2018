data = ["[1518-02-28 23:56] Guard #887 begins shift",
...
def get_guardId(action):
	return re.findall(r"#(\d+)", action)[0]

guards = defaultdict(list)
current_active_guard = "" 
for row in data:
	datetime_str, action = row.split("]")
	dt = datetime.strptime(datetime_str, '[%Y-%m-%d %H:%M')
	if "begins shift" in action:
		current_active_guard = get_guardId(action)
	elif "falls asleep" in action:
		d_start = dt
	elif "wakes up" in action:
		guards[current_active_guard].append((d_start.minute, dt.minute))

the_guard, most_frequent_minute = 0, 0
for guard in guards:
	max_minute_count, the_minute = 0, 0
	for minute in range(60):
		count_for_current_minute = sum([1 for start,stop in guards[guard] if start <= minute < stop])
		if count_for_current_minute > max_minute_count:
			max_minute_count = count_for_current_minute
			the_minute = minute
		most_frequent_minute = max_minute_count 
		the_guard = guard
		the_minute = the_minute 
	print(guard, 'is most active at minute', the_minute, 'times', max_minute_count) 
print(int(the_guard), most_frequent_minute, the_minute)
